DROP PROCEDURE IF EXISTS dbo.usp_add_UserDevice;
...
BEGIN
	DECLARE _ID INT;
	SELECT ud.ID INTO _ID FROM dbo.tbl_mstr_UserDevice ud WHERE ud.DeviceID = _DeviceID;
    ...
    INSERT INTO dbo.tbl_mstr_UserDevice(ID, DeviceID, Uri, DeviceType, DeviceVersion, AppVersion, InsertDate, UpdateDate)
    SELECT ID, DeviceID, Uri, DeviceType, DeviceVersion, AppVersion, NOW(), NOW() 
...
#CALL dbo.usp_add_UserDevice('DeviceID','Urid','DeviceType','DeviceVersion','AppVersion');
